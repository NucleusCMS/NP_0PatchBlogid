...
function getName() { return '0PatchBlogid'; }
function getAuthor() { return 'japan.nucleuscms.org'; }
function getURL() { return 'http://japan.nucleuscms.org/'; }
function getVersion() { return '0.2.5'; }
function getMinNucleusVersion() { return 250; }
function getDescription() { return "check \$blogid and \$archive"; }
...
function install() {
	...
	//Plugins sort
	$myid = $this->getID();
	$res = sql_query(sprintf('SELECT pid, porder FROM '.sql_table('plugin')));
	while($p = mysql_fetch_object($res)) {
		if ($p->pid == $myid) {
			sql_query(sprintf('UPDATE %s SET porder = %d WHERE pid = %d',
			sql_table('plugin'), 1, $myid));
		} else {
			sql_query(sprintf('UPDATE %s SET porder = %d WHERE pid = %d',
			sql_table('plugin'), $p->porder+1, $p->pid));
		}
	}
}
function event_PreSendContentType($arg) {
	...
	if (strtolower($arg['pageType']) !== 'skin') return;
	...
	if ($server_request_uri) {
		...
		if (preg_match('/page_/', $server_script_name)) {
			$temp_info = explode('page_', $server_script_name);
			$server_script_name = $temp_info[0] . 'page_' . intval($temp_info[1]);
			// ItemNaviEX breadcrumbslist fix
